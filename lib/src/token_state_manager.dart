import 'dart:async';

import 'package:flutter/foundation.dart';
import 'package:rxdart/rxdart.dart';

part './token_state.dart';

extension _TokenStateManagerExtension on TokenState {
  T valueOf<T>(TokenState<T> state) {
    return state._value;
  }

  T? valueOrNullOf<T>(TokenState<T> state) {
    return state._valueOrNull;
  }

  void updateStateOf<T>(TokenState<T> state, T newValue) {
    state._add(newValue);
  }
}

/// A [TokenStateManager] is a class that holds a list of [TokenSubject]s that
/// can be used to manage the state of the view.
///
/// It must be extended and the [subjects] must be implemented with the list of
/// [TokenSubject]s that will be used to manage the state of the view.
///
/// Example of usage:
/// ```dart
/// class ComboSuggestionBloc extends TokenStateManager {
///   ComboSuggestionBloc({
///     required this.getSnackByIdUC,
///     required this.addSnackToCartUC,
///     required String snackId,
///     required String comboSuggestionId,
///   }) {
///     on(
///       _fetchOptions(snackId, comboSuggestionId),
///       onData: (value) => updateStateOf(onNewStateSubject, value),
///     );

///     on(
///       onAddSnackSubject,
///       onData: _addSnack,
///     );
///   }

///   final GetSnackBarItemByIdUC getSnackByIdUC;
///   final AddSnackToCartUC addSnackToCartUC;

///   final onNewStateSubject = TokenState<ComboSuggestionState>();
///   final onNewActionSubject = TokenAction<ComboSuggestionAction>();
///   final onAddSnackSubject = TokenAction<Snack>();

///   @override
///   List<TokenSubject> get subjects => [
///         onNewStateSubject,
///         onNewActionSubject,
///         onAddSnackSubject,
///       ];

///   Stream<ComboSuggestionState> _fetchOptions(
///     String snackId,
///     String comboSuggestionId,
///   ) async* {
///     final values = await Future.wait([
///       getSnackByIdUC(
///         GetSnackBarItemByIdUCParams(
///           snackBarItemId: snackId,
///         ),
///       ),
///       getSnackByIdUC(
///         GetSnackBarItemByIdUCParams(
///           snackBarItemId: comboSuggestionId,
///         ),
///       )
///     ]);

///     final snack = values[0] as Snack;
///     final comboSnack = values[1] as ComboSnack;

///     yield Success(
///       snack: snack,
///       comboSnack: comboSnack,
///     );
///   }

///   Future<void> _addSnack(Snack snack) async {
///     await addSnackToCartUC(
///       AddSnackToCartUCParams(
///         snackPick: SnackPick(
///           cartUniqueId: getNewUniqueId(),
///           snackId: snack.id,
///         ),
///       ),
///     );
///     onNewActionSubject(
///       CloseDialogAfterAddSnack(
///         snack: snack,
///       ),
///     );
///   }
/// }
/// ```
abstract class TokenStateManager {
  final CompositeSubscription _subscriptions = CompositeSubscription();

  /// The list of [TokenSubject]s that will be used to manage the state of the
  /// view. It must be implemented.
  List<TokenSubject> get subjects;

  /// Listens to a [Stream] and adds the events to the [_subscriptions] list.
  ///
  /// It is a wrapper for the [Stream.listen] method.
  void on<T>(
    Stream<T> stream, {
    void Function(T)? onData,
    Function? onError,
    void Function()? onDone,
    bool? cancelOnError,
  }) {
    stream
        .listen(
          onData,
          onError: onError,
          onDone: onDone,
          cancelOnError: cancelOnError,
        )
        .addTo(_subscriptions);
  }

  void onVoid(
    Stream<void> stream, {
    void Function()? onData,
    Function? onError,
    void Function()? onDone,
    bool? cancelOnError,
  }) {
    on<void>(
      stream,
      onData: (_) => onData?.call(),
      onError: onError,
      onDone: onDone,
      cancelOnError: cancelOnError,
    );
  }

  /// Returns the value of a [TokenState].
  ///
  /// It is a wrapper for the [BehaviorSubject.value] getter.
  T valueOf<T>(TokenState<T> state) {
    return state.valueOf(state);
  }

  /// Returns the value of a [TokenState] or null if it has no value.
  ///
  /// It is a wrapper for the [BehaviorSubject.valueOrNull] getter.
  T? valueOrNullOf<T>(TokenState<T> state) {
    return state.valueOrNullOf(state);
  }

  /// Updates the value of a [TokenState].
  ///
  /// It is a wrapper for the [BehaviorSubject.add] method.
  void updateStateOf<T, S extends T>(TokenState<T> state, S newValue) {
    state._add(newValue);
  }

  /// Closes all the [TokenSubject]s.
  @mustCallSuper
  void dispose() {
    for (final subject in subjects) {
      subject.close();
    }
    _subscriptions.dispose();
  }
}
